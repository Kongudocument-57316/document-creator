...
export function numToTamilWords(num: number): string {
  ...
  if (rupees > 0) {
    result += convertNumberToTamilWords(rupees) + " "
  }
...
// Export the function with the name that's being imported in the form component
export const convertToTamilWords = (num: number | string): string => {
...
// Keep the original function name as well for backward compatibility
export const convertToTamilNumber = convertToTamilWords
